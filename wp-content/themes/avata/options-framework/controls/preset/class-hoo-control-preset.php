 ...
 */
class Hoo_Control_Preset extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		if ( ! class_exists( 'Hoo_Custom_Build' ) || ! Hoo_Custom_Build::is_custom_build() ) {
			wp_register_script( 'hoo-set-setting-value', trailingslashit( Hoo::$url ) . 'controls/preset/set-setting-value.js' );
			wp_enqueue_script( 'hoo-preset', trailingslashit( Hoo::$url ) . 'controls/preset/preset.js', array( 'jquery', 'customize-base', 'hoo-set-setting-value' ), false, true );
			wp_enqueue_style( 'hoo-preset-css', trailingslashit( Hoo::$url ) . 'controls/preset/preset.css', null );
		}
	}
