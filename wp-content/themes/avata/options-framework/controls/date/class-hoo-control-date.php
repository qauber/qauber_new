 ...
 */
class Hoo_Control_Date extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		if ( class_exists( 'Hoo_Custom_Build' ) ) {
			...
			Hoo_Custom_Build::register_dependency( 'jquery-ui-datepicker' );
		}

		if ( ! class_exists( 'Hoo_Custom_Build' ) || ! Hoo_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'hoo-date', trailingslashit( Hoo::$url ) . 'controls/date/date.js', array( 'jquery', 'customize-base', 'jquery-ui-datepicker' ), false, true );
			wp_enqueue_style( 'hoo-date-css', trailingslashit( Hoo::$url ) . 'controls/date/date.css', null );
		}
	}
