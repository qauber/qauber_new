wp.customize.controlConstructor['hoo-code'] = wp.customize.Control.extend({
	...
	hooLoader: function( forceLoad ) {
		var control  = this,
			waitTime = 100,
			i;

		if ( _.isUndefined( window.hooControlsLoader ) ) {
			window.hooControlsLoader = {
				queue: [],
				done: [],
				busy: false
			};
		...
		if ( -1 !== window.hooControlsLoader.done.indexOf( control.id ) ) {
			return;
		}

		// Add this control to the queue if it's not already there.
		if ( -1 === window.hooControlsLoader.queue.indexOf( control.id ) ) {
			window.hooControlsLoader.queue.push( control.id );
		}

		// If we're busy check back again later.
		if ( true === window.hooControlsLoader.busy ) {
			setTimeout( function() {
				control.hooLoader();
			}, waitTime );
			return;
		...
		if ( true === forceLoad || false === window.hooControlsLoader.busy ) {

			// Set to busy.
			window.hooControlsLoader.busy = true;

			// Init the control JS.
			control.initHooControl();
			jQuery( control.container.find( '.hoo-controls-loading-spinner' ) ).hide();

			// Mark as done and remove from queue.
			window.hooControlsLoader.done.push( control.id );
			i = window.hooControlsLoader.queue.indexOf( control.id );
			window.hooControlsLoader.queue.splice( i, 1 );

			// Set busy to false after waitTime has passed.
			setTimeout( function() {
				window.hooControlsLoader.busy = false;
			}, waitTime );
			return;
		}

		if ( control.id === window.hooControlsLoader.queue[0] ) {
			control.hooLoader( true );
		}
	},
	...
	initHooControl: function() {
		...
		var control  = this,
		    element  = control.container.find( '.hoo-codemirror-editor' ),
		...
		if ( ! _.isUndefined( control.params.choices.height ) ) {
			...
			if ( ! isNaN( height ) ) {
				container = control.container.find( '.codemirror-hoo-wrapper' );
