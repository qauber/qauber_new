 ...
 */
class Hoo_Control_Color_Palette extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		if ( ! class_exists( 'Hoo_Custom_Build' ) || ! Hoo_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'hoo-color-palette', trailingslashit( Hoo::$url ) . 'controls/color-palette/color-palette.js', array( 'jquery', 'customize-base', 'jquery-ui-button' ), false, true );
			wp_enqueue_style( 'hoo-color-palette-css', trailingslashit( Hoo::$url ) . 'controls/color-palette/color-palette.css', null );
		}
	}
	...
	public function to_json() {
		...
		if ( ! isset( $this->json['choices']['colors'] ) || empty( $this->json['choices']['colors'] ) ) {
			$this->json['choices']['colors'] = Hoo_Helper::get_material_design_colors( 'primary' );
		}
