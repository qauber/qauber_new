wp.customize.controlConstructor['hoo-dimensions'] = wp.customize.Control.extend({
			...
			setting.bind( function( value ) {
				...
				_.each( ['top', 'bottom', 'left', 'right'], function( direction ) {
					if ( ! _.isUndefined( value[ direction ] ) ) {
						if ( false === control.hooValidateCSSValue( value[ direction ] ) ) {
							subs[ direction ] = dimensionshooL10n[ direction ];
						} else {
				...
				if ( ! _.isEmpty( subs ) ) {
					message = dimensionshooL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
