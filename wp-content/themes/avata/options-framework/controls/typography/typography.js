wp.customize.controlConstructor['hoo-typography'] = wp.customize.Control.extend({
	...
	getFonts: function() {
		...
		if ( ! _.isUndefined( window[ 'hooFonts' + control.id ] ) ) {
			return window[ 'hooFonts' + control.id ];
		}
		if ( ! _.isUndefined( hooAllFonts ) ) {
			return hooAllFonts;
		}
		return {
			google: [],
