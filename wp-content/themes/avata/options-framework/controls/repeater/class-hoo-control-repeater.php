 ...
 * Repeater control
 */
class Hoo_Control_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'avata' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'avata' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'hoo-repeater', trailingslashit( Hoo::$url ) . 'controls/repeater/repeater.js', array( 'jquery', 'customize-base', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
		wp_enqueue_style( 'hoo-repeater-css', trailingslashit( Hoo::$url ) . 'controls/repeater/repeater.css', null );
	}
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'avata' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
						...
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									<# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}</span><# } #>
									<# if ( field.description ) { #><span class="description customize-control-description">{{{ field.description }}}</span><# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'avata' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
					...
					<div class="customize-control-hoo-editor">
			...
			<div class="customize-control-content">
				...
				<textarea {{{ data.inputAttrs }}} class="hidden" {{{ data.link }}}>{{ data.value }}</textarea>
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="hoo-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'avata' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'avata' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'avata' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'avata' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'avata' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'avata' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'avata' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="hoo-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'avata' ); ?>" >
									<# if ( field.default ) { #>
								...
								<div class="actions">
									...
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'avata' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'avata' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'avata' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'avata' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'avata' ); ?></button>
				</div>
			</li>
		</script>
		<?php
