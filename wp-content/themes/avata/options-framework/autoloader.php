<?php
if ( ! function_exists( 'hoo_autoload_classes' ) ) {
	/**
	 * The Hoo class autoloader.
	 * @param string $class_name The name of the class we're trying to load.
	 */
	function hoo_autoload_classes( $class_name ) {
		$paths = array();
		if ( 0 === stripos( $class_name, 'Hoo' ) ) {

			// Build the filename.
			$filename = 'class-' . strtolower( str_replace( '_', '-', $class_name ) ) . '.php';

			// Break class-name is parts.
			$name_parts = explode( '_', str_replace( 'Hoo_', '', $class_name ) );

			// Handle modules loading.
			if ( isset( $name_parts[0] ) && 'Modules' === $name_parts[0] ) {
				$path  = dirname( __FILE__ ) . '/modules/';
				$path .= strtolower( str_replace( '_', '-', str_replace( 'Hoo_Modules_', '', $class_name ) ) ) . '/';
			...
			if ( isset( $name_parts[0] ) ) {
				...
				if ( 'Control' === $name_parts[0] ) {
					...
					$path .= strtolower( str_replace( '_', '-', str_replace( 'Hoo_Control_', '', $class_name ) ) ) . '/';
				...
				if ( 'Settings' === $name_parts[0] ) {
					$path  = dirname( __FILE__ ) . '/controls/';
					$path .= strtolower( str_replace( '_', '-', str_replace( array( 'Hoo_Settings_', '_Setting' ), '', $class_name ) ) ) . '/';
			...
			$substr   = str_replace( 'Hoo_', '', $class_name );
	...
	// Run the autoloader.
	spl_autoload_register( 'hoo_autoload_classes' );
} // End if().
