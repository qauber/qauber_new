 ...
 */
final class Hoo_Fonts_Google {

	/**
	 * The Hoo_Fonts_Google instance.
	...
	private function __construct() {
		...
		$this->google_fonts = Hoo_Fonts::get_google_fonts();
	 ...
	 *
	 * @return object Hoo_Fonts_Google
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new Hoo_Fonts_Google();
		}
	...
	public function generate_google_font( $args ) {
		...
		if ( isset( $args['type'] ) && 'hoo-typography' === $args['type'] ) {

			// Get the value.
			$value = Hoo_Values::get_sanitized_field_value( $args );
			...
			if ( ! Hoo_Fonts::is_google_font( $value['font-family'] ) ) {
				return;
			}
			...
			if ( isset( $value['subsets'] ) ) {
				...
				// Add the subset directly to the array of subsets in the Hoo_GoogleFonts_Manager object.
