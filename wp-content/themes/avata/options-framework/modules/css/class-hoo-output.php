...
class Hoo_Output {
	...
	/**
	 * The Hoo configuration used in the field.
	 *
	 * @access protected
	 * @var string
	 */
		...
		if ( isset( $output['value_pattern'] ) && ! empty( $output['value_pattern'] ) ) {
			if ( is_string( $output['value_pattern'] ) ) {
				...
				if ( isset( $output['pattern_replace'] ) && is_array( $output['pattern_replace'] ) ) {
					...
					$option_name = false;
					if ( isset( Hoo::$config[ $this->config_id ] ) ) {
						$config = Hoo::$config[ $this->config_id ];
	...
	protected function process_property_value( $property, $value ) {
		$properties = apply_filters( "options-framework/{$this->config_id}/output/property-classnames", array(
			'font-family'         => 'Hoo_Output_Property_Font_Family',
			'background-image'    => 'Hoo_Output_Property_Background_Image',
			'background-position' => 'Hoo_Output_Property_Background_Position',
		) );
