 ...
 * Handles CSS output.
 */
final class Hoo_Modules_CSS_Generator {
	...
	private function __construct() {
		if ( is_null( self::$google_fonts ) ) {
			self::$google_fonts = Hoo_Fonts::get_google_fonts();
		}
		if ( is_null( self::$backup_fonts ) ) {
			self::$backup_fonts = Hoo_Fonts::get_backup_fonts();
		}
	}
	...
	public static function css( $field ) {
		...
		self::$value = Hoo_Values::get_sanitized_field_value( $field );

		// Find the class that will handle the outpout for this field.
		$classname = 'Hoo_Output';
		$field_output_classes = apply_filters( "options-framework/{$field['hoo_config']}/output/control-classnames", array(
			'hoo-background'  => 'Hoo_Output_Field_Background',
			'hoo-dimensions'  => 'Hoo_Output_Field_Dimensions',
			'hoo-typography'  => 'Hoo_Output_Field_Typography',
			'hoo-multicolor'  => 'Hoo_Output_Field_Multicolor',
		) );
		if ( array_key_exists( self::$field_type, $field_output_classes ) ) {
			$classname = $field_output_classes[ self::$field_type ];
		}
		$obj = new $classname( $field['hoo_config'], self::$output, self::$value );
	...
	public static function styles_parse( $css = array() ) {
		...
		// Pass our styles from the options-framework/styles_array filter.
		$css = apply_filters( 'options-framework/styles_array', $css );
