...
class Hoo_Field {
	...
	/**
	 * The ID of the hoo_config we're using.
	 *
	 * @see Hoo_Config
	 * @access protected
	 * @var string
	 */
	protected $hoo_config = 'global';
	 ...
	 *
	 * @see https://hoo.org/docs/arguments/output
	 * @access protected
	 * @var array
	 */
	 ...
	 *
	 * @see https://hoo.org/docs/arguments/js_vars
	 * @access protected
	 * @var array
	 */
	 ...
	 *
	 * @see https://hoo.org/docs/arguments/variables
	 * @access protected
	 * @var array
	 */
	 ...
	 * Then it adds the field to Hoo::$fields.
	 *
	 * @access public
	 * @param string $config_id    The ID of the config we want to use.
	 *                             Defaults to "global".
	 *                             Configs are handled by the Hoo_Config class.
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		...
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			unset( $args['setting'] );
			error_log( 'Hoo: Typo found in field ' . $args['settings'] . ' ("setting" instead of "settings").' );
		}
		...
		$args['hoo_config'] = $config_id;

		$this->hoo_config = ( '' === $config_id ) ? 'global' : trim( esc_attr( $config_id ) );

		// Get defaults from the class.
		$defaults = get_class_vars( __CLASS__ );

		// Get the config arguments, and merge them with the defaults.
		$config_defaults = ( isset( Hoo::$config['global'] ) ) ? Hoo::$config['global'] : array();
		if ( 'global' !== $this->hoo_config && isset( Hoo::$config[ $this->hoo_config ] ) ) {
			$config_defaults = Hoo::$config[ $this->hoo_config ];
		}
		...
		$whitelisted = apply_filters( "options-framework/{$this->hoo_config}/fields/properties_whitelist", array(
	 ...
	 * @access protected
	 */
	protected function set_hoo_config() {

		$this->hoo_config = esc_attr( $this->hoo_config );
	...
	protected function set_settings() {
		...
		if ( ! is_array( $this->settings ) ) {
			$this->settings = array(
				'hoo_placeholder_setting' => $this->settings,
			);
		}
		$settings = array();
		...
		if ( isset( $this->settings['hoo_placeholder_setting'] ) ) {
			$this->settings = $this->settings['hoo_placeholder_setting'];
		}
	...
	protected function set_active_callback() {
		...
		if ( ! empty( $this->required ) ) {
			$this->active_callback = array( 'Hoo_Active_Callback', 'evaluate' );
			return;
		}
	...
	protected function set_sanitize_callback() {
		...
		$default_callbacks = array(
			'hoo-multicheck'       => array( 'Hoo_Sanitize_Values', 'multicheck' ),
			'hoo-typography'       => array( 'Hoo_Sanitize_Values', 'typography' ),
		);
	...
	protected function set_output() {
		...
		foreach ( $this->output as $output ) {
			if ( ! isset( $output['element'] ) || ( ! isset( $output['property'] ) && ! in_array( $this->type, array( 'hoo-typography', 'hoo-background' ), true ) ) ) {
				continue;
			}
