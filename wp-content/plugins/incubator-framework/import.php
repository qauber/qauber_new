...
set_time_limit(-1);
if(isset($_POST['url'])) {
	$_POST['url'] = 'http://keydesign-themes.com/incubator/import/' . $_POST['url'];
	...
	if( isset($keydesign_check_files['erros']) ){
		...
		$json_file = wp_remote_get($_POST['url'] . '/theme_options.json');
		if( $json_file['response']['code'] == '200' && !empty($json_file['body']) ){
		    update_option( 'redux_ThemeTek', json_decode($json_file['body'], true), '', 'yes' );
		}
