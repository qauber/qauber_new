...
if (!function_exists("nxs_getURL")){ function nxs_getURL($options, $postID, $addURLParams='') { global $nxs_SNAP; $gOptions = $nxs_SNAP->nxs_options; //prr($options);
...
if (!function_exists("nxssnap_enqueue_scripts")) { function nxssnap_enqueue_scripts(){  if (function_exists('get_current_screen')) $screen = get_current_screen(); else return; // prr($screen->id); // prr($screen);       snapautoposter_page_nxssnap-post  
...
if (!function_exists("nxs_admin_footer_pp")) {  function nxs_admin_footer_pp() { ?><div id="nxs_gPopupCntWrap" style="min-height: 300px;"><div id="nxs_gPopupContent"></div></div><?php }}
...
if (!function_exists('nxs_chckRmImage')){function nxs_chckRmImage($url, $chType='head'){ if( ini_get('allow_url_fopen')=='1' && @getimagesize($url)!==false) return true;
  $hdrsArr = nxs_getNXSHeaders(); $nxsWPRemWhat = 'wp_remote_'.$chType; $url = str_replace(' ', '%20', $url); $rsp  = $nxsWPRemWhat($url, array('headers' => $hdrsArr));  
   ...
   <div class="nxsPostEd_ElemWrap" id="altFormatIMG<?php echo $nt.$ii; ?>" style="<?php echo $hide?'display:none;':''; ?>"><div class="nxsPostEd_ElemLabel" style="display: inline;"><?php _e('Image to use:', 'social-networks-auto-poster-facebook-twitter-g') ?></div>
     <div class="nxsPostEd_Elem" style="display: inline;"><input type="checkbox" class="isAutoImg" <?php if ($imgToUse=='') { ?>checked="checked"<?php } ?>  id="isAutoImg-<?php echo $nt; ?><?php echo $ii; ?>" name="<?php echo $nt; ?>[<?php echo $ii; ?>][isAutoImg]" value="A"/> <?php _e('Auto', 'social-networks-auto-poster-facebook-twitter-g'); ?>
                  ...
                  <?php } else { ?><?php } ?>
                    ...
                    <input type="hidden" name="<?php echo $nt; ?>[<?php echo $ii; ?>][imgToUse]" class="nxsEdElem" value="<?php echo $imgToUse ?>" id="imgToUse-<?php echo $nt; ?><?php echo $ii; ?>" /> 
     </div>
   </div> 
<?php }}
if (!function_exists('nxs_showURLToUseDlg')){ function nxs_showURLToUseDlg($nt, $ii, $urlToUse){ ?>
 <div class="nxsPostEd_ElemWrap" style=""><div class="nxsPostEd_ElemLabel" style="display: inline;"><?php _e('URL to use:', 'social-networks-auto-poster-facebook-twitter-g') ?></div>
   <div class="nxsPostEd_Elem" style="display: inline;"><input type="checkbox" class="isAutoURL nxsEdElem" <?php if ($urlToUse=='') { ?>checked="checked"<?php } ?>  id="isAutoURL-<?php echo $nt; ?><?php echo $ii; ?>" name="<?php echo $nt; ?>[<?php echo $ii; ?>][isAutoURL]" value="A"/> <?php _e('Auto', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Post URL or globally defined URL will be used', 'social-networks-auto-poster-facebook-twitter-g'); ?></i>                  
     <div class="nxs_prevURLDiv" <?php if (trim($urlToUse)=='') { ?> style="display:none;"<?php } ?> id="isAutoURLFld-<?php echo $nt; ?><?php echo $ii; ?>">
       <span><i><?php _e('Please note: This will trigger "Network will decide attachment info" on.', 'social-networks-auto-poster-facebook-twitter-g') ?><br/>
         <?php _e('Selected image and all other attachment URL settings will be ignored.', 'social-networks-auto-poster-facebook-twitter-g') ?>
       </i></span><br/>                      
       &nbsp;&nbsp;&nbsp;<?php _e('URL:', 'social-networks-auto-poster-facebook-twitter-g') ?> <input style="width:60%;max-width: 610px;" class="nxsEdElem" type="text" name="<?php echo $nt; ?>[<?php echo $ii; ?>][urlToUse]" value="<?php echo $urlToUse ?>" id="URLToUse-<?php echo $nt; ?><?php echo $ii; ?>" />                       
       </div>                  
...
if (!function_exists('nxs_LogIt')){ function nxs_LogIt($type, $action, $ntName, $ntType='', $title='', $extInfo='', $flt='snap', $uid=0){ global $wpdb; if (empty($uid)) { global $nxs_uid;  $uid = !empty($nxs_uid)?$nxs_uid:get_current_user_id(); }
   global $nxs_SNAP; if (isset($nxs_SNAP)) $options = $nxs_SNAP->nxs_options;  if (!empty($options) && !empty($options['numLogRows'])) $numLogRows = $options['numLogRows']; else $numLogRows = 1000; 
...
//## Format
if (!function_exists('nxs_makeURLParams')){ function nxs_makeURLParams($params) { $templ = nxs_getFromGlobalOpt('addURLParams'); if (empty($templ)) return false; 
...
function nxs_end_flush_ob(){ 
    if ( strpos($_SERVER["HTTP_USER_AGENT"], "facebookexternalhit") !== false || strpos($_SERVER["HTTP_USER_AGENT"], "Facebot") !== false || strpos($_SERVER["HTTP_USER_AGENT"], "ChXrome") !== false || strpos($_SERVER["HTTP_USER_AGENT"], "Google (") !== false || strpos($_SERVER["HTTP_USER_AGENT"], "LinkedInBot") !== false || strpos($_SERVER["HTTP_USER_AGENT"], "XING-contenttabreceiver") !== false || strpos($_SERVER["HTTP_USER_AGENT"], "Java/1.7.0_45") !== false) {
...
function nxs_ogtgCallback($content){ global $post, $nxs_SNAP;  
  if (stripos($content, 'og:title')!==false) $ogOut = "\r\n"; else {
    ...
    if (!empty($accs['fb']) && count($accs['fb'])>0) { $k = array_shift(array_slice($accs['fb'], 0, 1)); $ogappID = (!empty($k) && !empty($k['appKey']))  ? ('<meta property="fb:app_id" content="'.$k['appKey'].'"/>'."\r\n"):'';
...
if (!function_exists('nxs_postFromForm')){ function nxs_postFromForm($post, $networks, $isSilent=false){ global $nxs_snapAvNts; $postResults = '';
    if (!empty($post['mNts']) && is_array($post['mNts'])) { nxs_addToLogN('S', '-=== New Qiuck Post ===-', 'Form', count($post['mNts']).' Networks', print_r($post['mNts'], true));
      ...
      $message['pText'] = nxs_doSpin($_POST['mText']); $message['pTitle'] = nxs_doSpin($_POST['mTitle']);               
      //## Get URL info
    ...
    if (!empty($_POST['qpid'])) { $metaArr = get_post_meta($_POST['qpid'], '_nxs_snap_data', true ); $metaArr['nts'] = $nts; $metaArr['posts'][] = $postResultsArr;  update_post_meta($_POST['qpid'], '_nxs_snap_data', $metaArr); }    
...
//  prr($logToSend);
  ...
  //## Facebook
  if (is_array($networks['fb'])) foreach ($networks['fb'] as $ii=>$fbo) if ($fbo['riComments']=='1') {  $fbo['ii'] = $ii; $fbo['pType'] = 'aj';
  ...
  //## Twitter
  if (is_array($networks['tw'])) foreach ($networks['tw'] as $ii=>$fbo) if ($fbo['riComments']=='1') {  $fbo['ii'] = $ii; $fbo['pType'] = 'aj'; 
    foreach ($riPosts as $postID) {  
