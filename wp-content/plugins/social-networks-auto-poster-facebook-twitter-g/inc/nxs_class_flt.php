...
class nxs_Filters {
    ...
    public static function save_filter( $post_id ) { 
      ...
      if ( get_post_type( $post_id ) === 'nxs_filter' ) {        
         ...
         if (!empty($pvData['nxs_tags_names'])) { $nTags = array(); $toIns = array(); 
            ...
            if (!empty($pval['nxs_meta_key']) && $pval['nxs_meta_key']!='snap_isAutoPosted') {       
            ...
            } $jjj = 1; //prr($pval['nxs_count_term_compares']);
            if (!empty($pval['nxs_count_meta_compares']) && (int)$pval['nxs_count_meta_compares']>1) for( $jj = 2; $jj <= $pval['nxs_count_meta_compares']; $jj++ ) { 
               if (!empty($pval['nxs_meta_key_'.$jj]) && $pval['nxs_meta_key_'.$jj]!='snap_isAutoPosted') { $pm = array(); $jjj++;
            ...
            if (isset($pvData['nxs_tax_names']) && empty($pvData['nxs_meta_key'])) unset($pvData['nxs_tax_names']);
   ...
   <div class="nxs_tls_bdX">          
    ...
    public static function print_meta_section( $current_post, $metaSettings, $nt='', $ii='' ) { 
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_meta_compare_'. $i;  $jj = $i-1;    if ($i>1 && empty($metaSettings['post_meta'][$jj]["key"])) continue;   
          ...
          self::printSelect( "nxs".$nt.$ii."_meta_relation", self::makeInputName("nxs_meta_relation$postfix", $nt, $ii ), false, $relation_options, !empty($metaSettings['post_meta'][$jj]["relation"])?$metaSettings['post_meta'][$jj]["relation"]:'', !empty($current_post->ID)?$current_post->ID:0, false, 'notTknz');
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_term_compare_'. $i; if ($i>1 && empty($metaSettings["nxs_term_names$postfix"])) continue;
             echo '<div class="nxs_terms_panel"  id="nxs_term_namesDiv'.$nt.$ii.$postfix.'"><hr/>'; 
                ...
                echo '<div class="nxs_terms_leftPanel" style="display:'.(($i>1)?'inline-block':'none').';">'; 
                 ...
                 echo '<label class="field_title">'. __( 'Condition', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>';
                 //self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $relation_options, "nxs".$nt.$ii."_term_relation", !empty($metaSettings["nxs_term_relation$postfix"])?$metaSettings["nxs_term_relation$postfix"]:'', false, false, self::makeInputName("nxs_term_relation$postfix", $nt, $ii ), 'hiu'); 
                 
                 self::printSelect( "nxs".$nt.$ii."_term_relation", self::makeInputName("nxs_term_relation$postfix", $nt, $ii ), false, $relation_options, !empty($metaSettings["nxs_term_relation$postfix"])?$metaSettings["nxs_term_relation$postfix"]:'', !empty($current_post->ID)?$current_post->ID:0, false, 'notTknz');
                 echo '</div>';
...
function get_posts_ids_by_filter( $filter ) { $args = array('fields' => 'ids'); //prr($filter);
    ...
    */
    ...
    $args['suppress_filters'] = false;  $postsRet = nxs_get_posts( $args ); $post_ids = $postsRet['p']; $qu = $postsRet['q']; // prr($args);  prr($postsRet);      
    // nxs_LogIt('I', 'Reposter Check X', 'Reposter', '', '', '-=[ '.print_r($args, true).' ]=-'."<br/>\r\n".print_r($postsRet, true).'');
    // prr($qu);  prr($post_ids);  prr($args);  prr($outInfo);
