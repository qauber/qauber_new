...
if (!function_exists("nxs_snapAjax")) { function nxs_snapAjax() { check_ajax_referer('nxsSsPageWPN'); $arg = ''; nxs_Filters::init(true);
  ...
  if ($_POST['nxsact']=='saveRpst'){ if (!empty($_POST['pid'])) { $pid = $_POST['pid'];  }
     ...
     if (!empty($pid)) { $flt = nxs_Filters::save_filter($pid);  $rpstrOpts = nxs_Filters::save_schinfo($pid); 
       if (!empty($_POST['resetStats'])) delete_post_meta($pid, 'nxs_rpstr_stats'); $stats = nxs_Filters::getStats($pid, '', $rpstrOpts); if ($newReposter) die('OK'); else  echo $stats['statsText']; 
     } else die('ERROR (PID)');
...
//## List sort
if (!function_exists('nxsLstSort')){function nxsLstSort($a, $b) { if ($a['do']=='2') $a['do'] = 1;  if ($b['do']=='2') $b['do'] = 1; 
...
if (!function_exists("nxs_show_noLibWrn")) { function nxs_show_noLibWrn($msg){ ?> <div style="border: 2px solid darkred; padding: 25px 15px 15px 15px; margin: 3px; background-color: #fffaf0;"> 
            ...
            <div style="padding-bottom: 5px;"><a href="https://www.nextscripts.com/snap-api/">SNAP Premium API libraries package</a> adds autoposting to:</div> <span class="nxs_txtIcon nxs_ti_gp">Google+</span>, <span class="nxs_txtIcon nxs_ti_pn">Pinterest</span>, <span class="nxs_txtIcon nxs_ti_rd">Reddit</span>, <span class="nxs_txtIcon nxs_ti_bg">Blogger</span>,&nbsp;&nbsp;<span class="nxs_txtIcon nxs_ti_yt">YouTube</span>,&nbsp;&nbsp;<span class="nxs_txtIcon nxs_ti_fp">Flipboard</span>, <span class="nxs_txtIcon nxs_ti_li">LinkedIn Groups</span><br><br>          
