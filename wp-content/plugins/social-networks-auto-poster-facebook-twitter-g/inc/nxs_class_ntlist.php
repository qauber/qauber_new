...
if (!class_exists('nxs_snapClassNT')) { class nxs_snapClassNT {    
    var $nt = array(); var $ntInfo = array(); var $noFuncMsg = ''; var $isMobile = false; var $isRepost = false;
    ...
    public function showNTGroup() { $cbo = count($this->nt); $this->doAuth();  ?> <div class="nxs_box" onmouseover="jQuery('.addMore<?php echo $this->ntInfo['code']; ?>').show();" onmouseout="jQuery('.addMore<?php echo $this->ntInfo['code']; ?>').hide();">
        ...
        <div class="nxs_box_inside"><?php $jj = 0;  if(!$this->checkIfFunc()) echo $this->noFuncMsg; else {  uasort($this->nt, 'nxsLstSort'); 
            foreach ($this->nt as $indx=>$pbo){ $jj++; if (!function_exists('ns_SMASV41') && $jj>1) break; $pbo['jj']=$jj; $pbo['cbo']=$cbo; $this->showNTLine($indx, $pbo); }
    ...
    function showNoAPIMsg($ii, $options){ ?> <div id="do<?php echo $this->ntInfo['code'].$ii; ?>Div" class="insOneDiv<?php echo " clNewNTSets"; ?>"><div style="border: 2px solid darkred; padding: 25px 15px 15px 15px; margin: 3px; background-color: #fffaf0;"> 
            ...
            <div style="padding-bottom: 5px;"><a href="https://www.nextscripts.com/snap-api/">SNAP Premium API libraries package</a> adds autoposting to:</div> <span class="nxs_txtIcon nxs_ti_gp">Google+</span>, <span class="nxs_txtIcon nxs_ti_pn">Pinterest</span>, <span class="nxs_txtIcon nxs_ti_ig">Instagram</span>, <span class="nxs_txtIcon nxs_ti_rd">Reddit</span>, &nbsp;&nbsp;<span class="nxs_txtIcon nxs_ti_yt">YouTube</span>,&nbsp;&nbsp;<span class="nxs_txtIcon nxs_ti_fp">Flipboard</span>, <span class="nxs_txtIcon nxs_ti_li">LinkedIn Groups</span><br><br>          
...
<b style="color: #008000">[Limited Time Only Offer]</b><br/> Get SNAP PRO Plugin for <b>Free</b> with the order of SNAP Premium API for WordPress</div>
        ...
        <div class="nsx_tab_container"><?php /* ######################## Account Tab ####################### */ ?>
          ...
          <?php if (!$isNew && function_exists('_make_url_clickable_cb') ) { ?>   
            <div id="nsx<?php echo $nt.$ii ?>_tab2" class="nsx_tab_content"> 
               <?php $this->showNTFilters($nt, $ii, $options); $this->showImgSizeChoice($nt, $ii, !empty($options['wpImgSize'])?$options['wpImgSize']:'full'); $this->addPostingDelaySelV4($nt, $ii, $options); $this->advTab($ii, $options); ?>
   ...
   <div style="width:100%;"><strong><?php _e('Username:Password', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</strong> </div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][proxyup]" style="width: 30%;" value="<?php echo htmlentities($options['proxy']['up'], ENT_COMPAT, "UTF-8"); ?>"/>
    ...
    public function elemKeySecret($ii,$lKey,$lSec,$key,$sec,$fnKey='appKey',$fnSec='appSec', $aurl='') { $nt = $this->ntInfo['lcode']; $ntU = $this->ntInfo['code']; $aurl = !empty($aurl)?' (<a style="font-size: 12px;" href="'.$aurl.'" target="_blank">'.$aurl.'</a>)':''; ?>
     <div style="width:100%;"><b style="font-size: 14px;"><?php echo $lKey; echo $aurl; ?> </b></div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][<?php echo $fnKey; ?>]" id="<?php echo $nt.$fnKey.$ii; ?>" class="nxAccEdElem" value="<?php echo htmlentities($key, ENT_COMPAT, "UTF-8"); ?>" /> 
     <div style="width:100%;"><b style="font-size: 14px;"><?php echo $lSec; ?>:</b></div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][<?php echo $fnSec; ?>]" id="<?php echo $fnSec.$ii; ?>" class="nxAccEdElem" value="<?php echo htmlentities($sec, ENT_COMPAT, "UTF-8"); ?>" /><?php               
    }
    ...
    public function showEditNTLine($ii, $pbo, $post) { if(!$this->checkIfFunc()) return;
      ...
      ?>
      <div id="dom<?php echo $this->ntInfo['code'].$ii; ?>Div"  class="nxs_ntGroupWrapper" style=" padding-bottom: 3px;<?php echo ($cbo>7 && $jj>5)?'display:none;':'';?>" class="<?php echo ($cbo>7 && $jj>5)?' showMore'.$this->ntInfo['code'].'"':''; ?>"  onmouseover="if(!jQuery('#nxsNTSetDiv<?php echo $this->ntInfo['code'].$ii; ?>').is(':visible')) jQuery('.showInlineMenu<?php echo $this->ntInfo['code'].$ii; ?>').show();jQuery(this).addClass('nxsHiLightBorder');" onmouseout="jQuery('.showInlineMenu<?php echo $this->ntInfo['code'].$ii; ?>').hide();jQuery(this).removeClass('nxsHiLightBorder');">
        ...
        <div class="nxsEdNTLineHeader" data-nt="<?php echo $this->ntInfo['lcode']; ?>" data-ii="<?php echo $ii; ?>" style="background-color: #F0F0F0; padding-top: 4px; padding-right: 10px; padding: 5px; margin-top: 3px; border: 1px solid #E0E0E0; ">
          ...
          <?php } else { ?>            
            <input value="0" name="<?php echo $this->ntInfo['lcode']; ?>[<?php echo $ii; ?>][do]" type="hidden" />             
        ...
        <?php //### TIme to Post
        if ($post->post_status != "publish" && function_exists('nxs_v4doSMAS5') ) { $pbo['postTime'] = get_post_time('U', false, $post->ID); nxs_v4doSMAS5($nt, $ii, $pbo); }      
