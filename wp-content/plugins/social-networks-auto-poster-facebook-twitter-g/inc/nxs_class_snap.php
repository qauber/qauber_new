...
if (!class_exists("nxs_SNAP")) { class nxs_SNAP {//## SNAP General Class         
  var $dbOptionsName = "nxsSNAPOptions"; var $dbNtsName = "nxsSNAPNetworks"; var $dbNtsNameU = "nxsSNAPNetworksU"; var $nxs_snapAvNts; var $nxs_options = ""; var $nxs_ntoptions = array(); var $nxs_accts = ""; var $nxs_acctsU = "";
  ...
  function getAPOptions() { global $nxs_skipSSLCheck, $blog_id; $options = get_option($this->dbOptionsName); $this->nxs_accts = get_option($this->dbNtsName); $this->nxs_acctsU = get_option($this->dbNtsNameU); 
    ...
    //## Backup
    $lBckTime = get_option('nxs_lBckTime'); if (empty($lBckTime) || $lBckTime<strtotime("-1 week")) { update_option('nxsSNAPNetworks_bck4', $this->nxs_accts); update_option('nxsSNAPOptions_bck4', $this->nxs_options); update_option('nxs_lBckTime', time()); }
  ...
  function saveNetworksOptions($networks, $options='') { //## Set or just save (=1) Options and Networks
     if (!empty($networks)) { 
         if (!current_user_can( 'manage_options' ) && current_user_can( 'haveown_snap_accss' )) { if ($networks!=1) $this->nxs_acctsU = $networks; update_option($this->dbNtsNameU, $this->nxs_acctsU); } 
            else { if ($networks!=1) $this->nxs_accts = $networks; update_option($this->dbNtsName, $this->nxs_accts); }         
...
&nbsp;&nbsp;<a style="font-size: 14px;" onclick="nxs_saveSiteSets('A', 'D');return false;" href="#">[Disable SNAP on all sites]</a><?php
  ...
  function showAccountsTab(){ global $nxs_snapAvNts, $nxsOne; $nxsOne = ''; $trrd=0; $nxs_snapThisPageUrl = home_url( add_query_arg( NULL, NULL ) ); $cst=strrev('enifed');  $isMobile = nxs_isMobile();
    ...
    $options = $this->nxs_options; $isNoNts = true;
 ...
 <br/>
     ...
     <!-- ##################### Open Graph #####################-->
            ...
            <input name="ogImgDef" style="width: 30%;" value="<?php if (isset($options['ogImgDef'])) _e(apply_filters('format_to_edit',$options['ogImgDef']), 'social-networks-auto-poster-facebook-twitter-g') ?>" />
             ...
             <input name="ogImgDef" style="width: 30%;" value="<?php if (isset($options['ogAuthorFB'])) _e(apply_filters('format_to_edit',$options['ogAuthorFB']), 'social-networks-auto-poster-facebook-twitter-g') ?>" />
              </div>              
           </div></div>    
            <!-- #### "Featured" Image ##### --> 
...
<?php } elseif(defined('NextScripts_UPG_SNAP_Version')) { ?> <br/><span style="color:red;">You have "SNAP Upgrade helper" installed, now please&nbsp;<a href="#" class="showLic">[Enter Activation Key]</a></span><br/><br/> <?php } ?><br/>
        ...
        <div class="nxscontainer">
          ...
          <div class="nxsleft">
            ...
            <a style="font-weight: normal; font-size: 16px; line-height: 24px;" target="_blank" href="<?php echo $nxs_snapThisPageUrl; ?>&do=test">Check HTTPS/SSL</a>&nbsp;&nbsp;<a style="font-weight: normal; font-size: 16px; line-height: 24px;" target="_blank" href="<?php echo $nxs_snapThisPageUrl; ?>&do=crtest">Show Cron Test Results</a><br/>
  ...
  function NS_SNAP_AddPostMetaTags() { global $post, $nxs_snapAvNts, $nxs_SNAP; $post_id = $post; if (is_object($post_id))  $post_id = $post_id->ID; 
       ...
       <?php /* ################## WHAT URL to USE */ ###################### ?>
          ...
          foreach ($nxs_snapAvNts as $avNt) { $clName = 'nxs_snapClass'.$avNt['code'];
             ...
             $cbo = 0; foreach ($ntClInst->nt as $indx=>$pbo){  // prr($indx); prr($pbo);
             ...
             if ($cbo==1) { ?>
               <?php $pbo['jj']=$indx; $pbo['cbo']=$cbo; $ntClInst->showEditNTLine($indx, $pbo, $post); ?>
             <?php } else {  ?>
...
<input type="hidden"  id="nxs_timeID" value="" />
  ...
  function addCustomBoxes() { global $nxs_SNAP; if (!isset($nxs_SNAP)) return; if ($nxs_SNAP->sMode['a']=='I') return; $options = $nxs_SNAP->nxs_options;  //## Add to Post, Page and Custom Post Types
     ...
     foreach ($post_types as $cptID=>$cptName) if (in_array($cptID, $options['fltrs']['nxs_post_type'])){ 
  ...
  function setSettingsFromPOST(){ $options = $this->nxs_options; $pvData = nxs_Filters::sanitize_data($_POST);
            ...
            if (isset($pvData['post_category']))  { $pk = $pvData['post_category']; if (!is_array($pk)) { $pk = urldecode($pk); parse_str($pk); } 
