...
*/
define( 'NextScripts_SNAP_Version' , '4.0.6' ); define( 'NextScripts_SNAP_Version_Date' , 'Oct 25, 2017' ); require_once "inc/nxs_functions_wp.php"; if(!defined( 'NXSSNAP_BASENAME' ) ) define( 'NXSSNAP_BASENAME', plugin_basename( __FILE__ ) );
...
if (true===nxs_doSystemInitCheck()) { // error_reporting(E_ALL); ini_set('display_errors', '1'); 
  ...
  global $nxs_snapAvNts, $nxs_SNAP, $nxs_snapSetPgURL, $nxs_snapThisPageUrl; /* $nxs_snapSetPgURL = nxs_get_admin_url('admin.php?page=nxssnap-accounts'); */  $nxs_snapSetPgURL = nxs_get_admin_url('admin.php?page=nxssnap');  $nxs_snapThisPageUrl = nxs_get_admin_url(str_ireplace('wp-admin/','',$_SERVER['REQUEST_URI']));
  ...
  do_action('nxs_actAfterGetAvNetworks');            //prr($nxs_snapAvNts, 'X2');     die();
  ...
  function nxs_initSNAP() { global $nxs_SNAP; 
    ...
    add_action('save_post', array($nxs_SNAP, 'NS_SNAP_SavePostMetaTags'));  //  prr($nxs_SNAP->nxs_options['nxsOG']); prr($_SERVER["HTTP_USER_AGENT"]); die();
  ...
  add_action('wp', 'nxs_activation'); //## Adds nxs_querypost_event to wp_schedule_event as nxsquery
  //## OG Tags
    
  //## Cron reposter action functions
  function cron_add_nxsAutPoster( $schedules ) { $schedules['nxsquery'] = array( 'interval' => 30, 'display' => __( 'NextScripts AutoPoster' )); return $schedules;} // Do this every 60 seconds
