...
;(function($) {
		...
		spotedImage.onload = function() {
			...
			self.element.delegate('span', 'click', function(event) {
				...
				if (widget.config.interactivity === 'none') {
					var htmlBuilt = $('<div id="asvc-hotspot-dot-'+counter+'" data-index="'+counter+'"><i class="delete-item fa fa-close"></i></div>');
				} else {
					var htmlBuilt = $('<div id="asvc-hotspot-dot-'+counter+'" data-index="'+counter+'"><i class="delete-item fa fa-close"></i></div>').addClass(widget.config.hiddenClass);
				}
				...
				$('body').trigger('asvc-hotspot-updated');
			});
			...
			if(typeof $.fn.draggable != 'undefined') {
				...
				if($('.HotspotPlugin_Hotspot').length > 0) {
					...
					$('body').on('asvc-hotspot-updated', function() {
	...
	Hotspot.prototype.popupWindow = function(index, currentData) {
		...
		for (var i = 0; i < dataStuff.length; i++) {
			var val = dataStuff[i],
				...
				input = '<label>'+val.property+'</label><textarea class="asvc-hotspot-'+val.property+'">'+defaultText+'</textarea>';
			
			if(val.property == 'Title') {
				input = '<label>'+val.property+'</label><input type="text" class="asvc-hotspot-'+val.property+'" value="'+defaultText+'" />';
			}
			
			$popupInnerHtml += '<div>'+input+'</div>';
		};
		
		$popupInnerHtml += '<a href="#" title="'+closeText+'" class="vc_general vc_ui-button vc_ui-button-default vc_ui-button-shape-rounded vc_ui-button-fw asvc-hotspot-close">'+closeText+'</a><a href="#" title="'+saveText+'" class="vc_general vc_ui-button vc_ui-button-action vc_ui-button-shape-rounded vc_ui-button-fw asvc-hotspot-save">'+saveText+'</a>';
		
		var $popupHtml = '<div class="asvc-hotspot-popup"><div class="asvc-hotspot-popup-title">'+popupTitle+'<a href="#" title="Close" class="asvc-hotspot-close Default-close"></a></div>'+$popupInnerHtml+'</div>';
		
		$('body').append($popupHtml);
		
		$('.asvc-hotspot-save').on('click', function() {
			var $container = $(this).parents('.asvc-hotspot-popup'),
				dataBuild = {};
			
			for (var i = 0; i < dataStuff.length; i++) {
				var val = dataStuff[i];
				dataBuild[val.property] = $container.find('.asvc-hotspot-'+val.property).val();
			};
			
			$('.asvc-hotspot-popup').remove();
		...
		$('body').on('click', '.asvc-hotspot-close', function() {
			$('.asvc-hotspot-popup').remove();
		});
	};
	...
	Hotspot.prototype.getData = function() {
		...
		$('body').trigger('asvc-hotspot-initialized');
	};
	...
	Hotspot.prototype.beautifyData = function() {
		...
		for (var i = obj.length - 1; i >= 0; i--) {
			...
			if (this.config.interactivity === 'none') {
				var htmlBuilt = $('<div id="asvc-hotspot-dot-'+el.index+'" data-index="'+el.index+'"><i class="delete-item fa fa-close"></i></div>');
			} else {
				var htmlBuilt = $('<div id="asvc-hotspot-dot-'+el.index+'" data-index="'+el.index+'"><i class="delete-item fa fa-close"></i></div>').addClass(this.config.hiddenClass);
			}
